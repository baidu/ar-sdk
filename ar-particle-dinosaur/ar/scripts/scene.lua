...
function CREATE_SCENE()
    ...
    NEW_SCENE.__newindex = function(self, key, value)
        if(key == 'on_touch_event') then
            ...
            local ESCROLL_UP = 11
            self.entity:set_event_handler(ESCROLL, handler_id)
            self.entity:set_event_handler(ESCROLL_DOWN, handler_id)
            self.entity:set_event_handler(ESCROLL_UP, handler_id)
