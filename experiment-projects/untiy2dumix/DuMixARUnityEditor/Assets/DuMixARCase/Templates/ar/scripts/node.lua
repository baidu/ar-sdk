...
function LOAD_NODE()
	...
	function GET_NODE(scene ,name)
		...
		node._unity_click_bind_handler = {}
		-- click handler --
		local anony_func = function()
			for i = 1, #node._unity_click_bind_handler do
				node._unity_click_bind_handler[i]()
			end

			if(node._on_click ~= 0) then
				node._on_click()
			end
		end
		...
		node.entity:set_event_handler(0, handler_id)
		...
		node.__newindex = function(self, key, value) 
			...
			elseif(key == 'unity_click_binder') then
				table.insert(self._unity_click_bind_handler, value)
			else 
