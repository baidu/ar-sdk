...
float fictionalShadowDense = 0.0;
    ...
    for ( int i = 0; i < NUM_POINT_LIGHTS; i ++ ) { 
        ...
        #ifdef USE_SHADOWMAP 
        float shadow = all( bvec2( pointLight.shadow, directLight.visible ) ) ? getPointShadow( pointShadowMap[ i ], pointLight.shadowMapSize, pointLight.shadowBias, pointLight.shadowRadius, vPointShadowCoord[ i ] ) : 1.0;
        directLight.color *= shadow;
        fictionalShadowDense += (1.0 - shadow);
        #endif
    ...
    for ( int i = 0; i < NUM_SPOT_LIGHTS; i ++ ) { 
        ...
        #ifdef USE_SHADOWMAP 
        float shadow = all( bvec2( spotLight.shadow, directLight.visible ) ) ? getShadow( spotShadowMap[ i ], spotLight.shadowMapSize, spotLight.shadowBias, spotLight.shadowRadius, vSpotShadowCoord[ i ] ) : 1.0;
        directLight.color *= shadow;
        fictionalShadowDense += (1.0 - shadow);
        #endif 
    ...
    for ( int i = 0; i < NUM_DIR_LIGHTS; i ++ ) { 
        ...
        #ifdef USE_SHADOWMAP 
        float shadow = all( bvec2( directionalLight.shadow, directLight.visible ) ) ? getShadow( directionalShadowMap[ i ], directionalLight.shadowMapSize, directionalLight.shadowBias, directionalLight.shadowRadius, vDirectionalShadowCoord[ i ] ) : 1.0;
        directLight.color *= shadow;
        fictionalShadowDense += (1.0 - shadow);
        #endif 
    ...
    if (diffuseColor.a > 0.0) {
        ...
        gl_FragColor = vec4( outgoingLight, diffuseColor.a +
                            max((1.0 - ambientLightIntensity), 0.1) * fictionalShadowDense);
    }
    
         #ifdef USE_MATCAPMAP
             vec3 matCapColor = texture2D( matCapMap, vMatCapUv.xy ).rgb;
             gl_FragColor.xyz = gl_FragColor.xyz * matCapColor * 2.0;
         #endif
