...
float ShadowCalculation(vec4 fragPosLightSpace, float bias)
{
    ...
    float closestDepth = texture2D(Texture0, projCoords.xy).r;
    // 取得当前片元在光源视角下的深度
...
vec3 calculateDirectionalLight(DirectionalLight light) {
    ...
    vec3 ambientColor = ambient * vec3(texture2D(Texture0, frag_texCoord));
    ...
    vec3 diffuseColor = 1.0 * diffuse * vec3(texture2D(Texture0, frag_texCoord));
    vec3 specularColor = 1.0 * specular * vec3(texture2D(Texture0, frag_texCoord));
...
vec3 calculatePointLight(PointLight light) {
    ...
    vec3 ambientColor = ambient * vec3(texture2D(Texture0, frag_texCoord));
    vec3 diffuseColor = diffuse *  vec3(texture2D(Texture0, frag_texCoord));
    vec3 specularColor = specular *  vec3(texture2D(Texture0, frag_texCoord));
...
vec3 calculateSpotLight(SpotLight light) {
    ...
    vec3 ambientColor = ambient * vec3(texture2D(Texture0, frag_texCoord));
    vec3 diffuseColor = diffuse * spotIntensity * vec3(texture2D(Texture0, frag_texCoord));
    vec3 specularColor = specular * spotIntensity * vec3(texture2D(Texture0, frag_texCoord));
...
void main() {
    ...
    //gl_FragColor = vec4(vec3(frag_texCoord.x, frag_texCoord.y, frag_texCoord.x), 1.0);
}
